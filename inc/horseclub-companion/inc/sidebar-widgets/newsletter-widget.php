...
class horseclub_newsletter_widget extends WP_Widget {
...
'horseclub_newsletter_widget',


// Widget name will appear in UI
esc_html__( '[ Horseclub ] Newsletter Form', 'horseclub' ), 

// Widget description
array( 'description' => esc_html__( 'Add footer newsletter signup form.', 'horseclub' ), ) 
);
...
?>
<div id="mc_embed_signup" class="horseclub-subscribe-newsletter newsletter-form">
	<form target="_blank" novalidate action="<?php echo esc_url( $actionurl ); ?>" id="mc-embedded-subscribe-form" method="post" class="navbar-form">
		...
		<input type="email" name="EMAIL" class="form-control" placeholder="<?php esc_html_e( 'Email address', 'horseclub' ); ?>" onfocus="this.placeholder = ''" onblur="this.placeholder = 'Email address'" required>
...
if ( isset( $instance[ 'title' ] ) ) {
	...
	$title = esc_html__( 'Newsletter', 'horseclub' );
}
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'actionurl' ) ); ?>"><?php esc_html_e( 'Action URL:' ,'horseclub'); ?></label> 
...
} // Class horseclub_newsletter_widget ends here


// Register and load the widget
function horseclub_newsletter_load_widget() {
	register_widget( 'horseclub_newsletter_widget' );
}
add_action( 'widgets_init', 'horseclub_newsletter_load_widget' );