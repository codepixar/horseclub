...
// Final Class
final class Horseclub {

	// Theme Version
	private $horseclub_version = '1.0';
	...
	private function setup() {
		...
		$enqueu = new Horseclub_Enqueue();
		$enqueu->scripts = $this->enqueue() ;
		$enqueu->horseclub_scripts_enqueue_init() ;

	}
	// Theme Support
	public function support() {
		// content width
        $GLOBALS['content_width'] = apply_filters( 'horseclub_content_width', 751 );

        
        // text domain for translation.
        load_theme_textdomain( 'horseclub', HORSECLUB_DIR_PATH . '/languages' );
		...
		add_image_size( 'horseclub_blogsect_post_thumb', 295, 211, true );
		
		// Custom thumbnails size for widget post
		add_image_size( 'horseclub_widget_post_thumb', 75, 75, true );

		// Custom  thumbnails size for next prev thumb
		add_image_size( 'horseclub-np-thumb', 59, 59, true );
        ...
        register_nav_menus( array(
            'primary-menu'   => esc_html__( 'Primary Menu', 'horseclub' )
        ) );
	...
	private function enqueue() {
		...
		$cssPath = HORSECLUB_DIR_CSS_URI;
		$jsPath  = HORSECLUB_DIR_JS_URI;
		...
		$scripts = array(
			'style' => array(
				...
				array(
					'handler'		=> 'horseclub-main',
					'file' 			=> $cssPath.'main.css',
					'dependency' 	=> array(),
					'version' 		=> $this->horseclub_version,
				),
				array(
					'handler'		=> 'horseclub-style',
	...
	private function google_font() {
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'horseclub' ) ) {
	...
	private function customizer_init() {
		...
		// Instantiate horseclub theme customizer
		$horseclub_theme_customizer = new horseclub_theme_customizer();
	}
	...
	public function epsilon_quickie() {
			...
			'links' => array(
				array(
					'link_to'   => 'horseclub_options_panel',
	...
	public function set_elementor_flag() {
		update_option( 'horseclub_had_elementor', 'no' );
	}
	...
	public function elementor_desiable_default_style() {
		...
		$nonce = $_POST['nonce'];
		if ( ! wp_verify_nonce( $nonce, 'horseclub-elementor-notice-nonce' ) ) {
			return;
		}
		$reply = $_POST['reply'];
		if ( ! empty( $reply ) ) {
			...
			update_option( 'horseclub_had_elementor', 'yes' );
		}
		die();
	...
	public function enqueue_elementor_theme_default_style() {
		...
		if ( $disabled_color_schemes === 'yes' && $disabled_typography_schemes === 'yes' ) {
			wp_enqueue_style( 'horseclub-elementor-default-style',  HORSECLUB_DIR_CSS_URI. 'elementor-default-element-style.css', array(), $this->horseclub_version );
		}
	}
		...
		if( $had_elementor == 'no' && self::check_elementor_preview_page() ) {
			...
			wp_localize_script(
				...
				array(
					...
					'nonce'   => wp_create_nonce( 'horseclub-elementor-notice-nonce' ),
				)
			);
		}
